...
from .jpdb import JPDB, JPDB_Anki, JPDB_Note
...
class JPDB_SearchWidget(aqt.QWidget):
    def __init__(
        ...
        self.decks_list = JPDB_Anki.decks().all_names()
    ...
    def _on_generate_pressed(self) -> None:
        ...
        JPDB_Anki.add_note(jpdb_note, self.current_deck)
...
class Anki_SearchWidget(aqt.QWidget):
    def __init__(
        ...
        self.editor = aqt.editor.Editor(JPDB_Anki.window(), editor_widget, self)
    ...
    def on_search_pressed(self):
        ...
        self.notes_id = JPDB_Anki.find_notes(query)
        notes_info = []
        for note_id in self.notes_id:
            note_name = JPDB_Anki.collection().get_note(note_id).fields[0]
    ...
    def on_note_selected(self):
        ...
        self.editor.set_note(JPDB_Anki.collection().get_note(note_id))
    ...
    def on_reposition_action(self):
        note_id = self.notes_id[self.query_results_list.currentIndex().row()]
        cards_id = JPDB_Anki.get_cards_of_note(note_id)

        for card_id in cards_id:
            card = JPDB_Anki.collection().get_card(card_id)
            if card.type > 0:  # if not new
                continue

            card.due = 0
            JPDB_Anki.collection().update_card(card)
